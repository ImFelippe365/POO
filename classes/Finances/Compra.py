"""Esse classe representa as compras realizadas pelos atendente, seus metodos comportão a manipulação
da lista de itens nela"""
import os
...
class Compra:
    ...
    def __init__(self, id, metodoPagamento,conta ,valor_total=0, lista_itens=[]):
        self.__id = id
    ...
    def adicionar_item_na_lista(self, item):
        if item.get_quantidade() <= item.get_produto().get_quantidade_estoque():
            ...
            print(Fore.RED+Style.BRIGHT+"[!] Quantidade insuficiente no estoque!"+Fore.RESET+Style.RESET_ALL)
